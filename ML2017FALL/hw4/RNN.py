...
def Build_model(SEQUENCES_SIZE, EMB_INPUT_SIZE, HIDDEN_DIM, embedding_matrix=[]):
    ...
    # LSTM
    RNN_cell = LSTM(HIDDEN_DIM,return_sequences=False,dropout=0.4)
...
#    X_raw, y_raw, word_index = Encoding(X_raw_seq,y_raw_cat,EMB_SEQ_SIZE)
#    X_train,X_test,y_train,y_test = train_test_split(X_raw,y_raw,train_size=0.7,shuffle=True) 
...
#        model = Build_model(EMB_SEQ_SIZE,EMB_INPUT_SIZE,HIDDEN_DIM,embedding_matrix)
    ...
    ## Encoding
    ob, ob_id, word_index = Encoding(ob, ob_id, SEQUENCES_SIZE) 
#    ob, ob_id, word_index = Encoding(ob, ob_id,EMB_SEQ_SIZE)
    ## Submission
    print("Submission ...")
